# this is just to fix something i fucked up in my operating system
import os
...
def least_squares(targets, inputs):
    # it generally makes sense to define something like this as a method
    # for a class, but we're doing everything functional here so i wont do that
    beta = targets @ inputs @ np.linalg.pinv(np.transpose(inputs) @ inputs)  
    return beta 
...
def predict(features, beta) -> int:
    ...
    return 0 if features @ beta < 0.5 else 1
